...
class PopularityBaseline():
    ...
    def evaluate(self, results, test_set):
        ...
        results = results.withColumn('id', fn.lit(1)).groupBy('id').agg(fn.collect_list('movieId').alias('top_movies'))
        
        UserMovies = data.groupBy('userId').agg(fn.collect_list('movieId').alias('liked_movies'))
